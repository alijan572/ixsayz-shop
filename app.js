...
state.visits += isAdminPage ? 0 : 1;
  ...
  async function startFirebaseSync() {
    ...
    try {
      ...
      const firebaseApp = initializeApp(window.IXSAYZ_FIREBASE_CONFIG);
      ...
      firestore.onSnapshot(remoteDocRef, (docSnap) => {
        ...
        const incoming = normalizeState(docSnap.data().state);
      ...
      toast("Firebase подключен: данные общие для всех устройств");
    } catch (error) {
      console.error(error);
      toast("Firebase не подключился. Проверьте firebase.js и Firestore.");
    }
  }
