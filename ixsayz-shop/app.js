(function () {
 ...
 const seed = {
  ...
  function clearBrowserStorage() {
    try {
      [storageKey, sessionKey, adminKey, "ixsayz-cart-v1"].forEach((key) => localStorage.removeItem(key));
      ...
      console.warn("Browser storage cleanup skipped", error);
    }
  }
  ...
  async function startFirebaseSync() {
    ...
    try {
      ...
      if (config.databaseURL) {
        ...
        const snap = await database.get(remoteDocRef);
        if (snap.exists() && snap.val()?.state) {
          state = normalizeState(snap.val().state);
        } else {
          await remoteSetDoc(remoteDocRef, { state: publicState(), updatedAt: remoteServerTimestamp() });
        }
        ...
        toast("Firebase Realtime Database подключена");
        return;
      }
      ...
      const snap = await firestore.getDoc(remoteDocRef);
      ...
      toast("Firebase Firestore подключена");
    } catch (error) {
      console.error(error);
      toast("Firebase не подключился. Проверьте правила базы и firebase.js");
    }
  }
  ...
  function bindShopEvents() {
    ...
    document.querySelectorAll("[data-open-product]").forEach((btn) => btn.addEventListener("click", () => showProductDetail(btn.dataset.openProduct)));
  ...
  if (hasFirebaseConfig) {
    app.innerHTML = `<section class="hero"><div class="hero-fallback"></div><div class="modal glass"><h3>IXSAYZ SHOP</h3><p class="muted">Подключаем Firebase...</p></div></section>`;
    startFirebaseSync();
  } else {
    app.innerHTML = `<section class="hero"><div class="hero-fallback"></div><div class="modal glass"><h3>Firebase не подключен</h3><p class="muted">Для рабочей версии нужен firebase.js. Данные в браузер не сохраняются.</p></div></section>`;
  }
}());
